...
//    assert (XMLHTTPRequest.isResponseReady(xmlHttpRequest));
  ...
  private void fireOnResponseReceivedImpl(RequestCallback callback) {
    ...
    if (XMLHTTPRequest.getReadyState(xmlHttpRequest) == XMLHTTPRequest.RECEIVING) {
    	if(callback instanceof RequestCallbackEx) {
    		Response response = createResponse(xmlHttpRequest);
    		((RequestCallbackEx)callback).onResponseReceiving(this, response);
    	}
    }
    else {
	    cancelTimer();
